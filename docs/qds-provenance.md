...
To refresh, copy the generated source file exactly, update every provenance field above, then run `QDS_ROOT=/path/to/design-system npm run qds:doctor`. The adapter scopes selectors for Shadow DOM and maps semantic variables; it does not alter generated token values.
