...
| Глубокий DOM-путь занимал фиксированную область и обрезался случайно | Breadcrumb горизонтально прокручивается к терминальному селектору; полный путь остаётся доступен | Пользователь сразу видит выбранный конечный элемент без скачков высоты панели |
