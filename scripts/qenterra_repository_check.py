...
PUBLIC_GENERATED_HEADER_EXTENSIONS = frozenset(('.c', '.cc', '.cpp', '.cs', '.css', '.dart', '.go', '.h', '.hpp', '.html', '.java', '.js', '.json', '.jsx', '.kt', '.kts', '.m', '.md', '.mjs', '.mm', '.php', '.plist', '.py', '.rb', '.rs', '.scala', '.sh', '.sql', '.swift', '.toml', '.ts', '.tsx', '.vue', '.xml', '.yaml', '.yml'))
...
def has_generated_header(path: Path) -> bool:
    if path.suffix.casefold() not in PUBLIC_GENERATED_HEADER_EXTENSIONS and path.suffix:
        return False
    lines: list[bytes] = []
    try:
        ...
        return False
    header = b"".join(lines).decode("utf-8", errors="ignore").casefold()
